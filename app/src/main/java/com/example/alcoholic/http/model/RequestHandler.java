package com.example.alcoholic.http.model;
...
import com.example.alcoholic.http.json.BooleanTypeAdapter;
import com.example.alcoholic.http.json.DoubleTypeAdapter;
import com.example.alcoholic.http.json.FloatTypeAdapter;
import com.example.alcoholic.http.json.IntegerTypeAdapter;
import com.example.alcoholic.http.json.ListTypeAdapter;
import com.example.alcoholic.http.json.LongTypeAdapter;
import com.example.alcoholic.http.json.StringTypeAdapter;
...
public final class RequestHandler implements IRequestHandler {
    ...
    public Object requestSucceed(LifecycleOwner lifecycle, Response response, Type type) throws Exception {
        ...
        if (String.class.equals(type)) {
            ...
            if (result instanceof ImgUploadBean){
                ImgUploadBean imgUploadBean = (ImgUploadBean) result;

                if (imgUploadBean.getStatus() == 200){
                    return result;
                }else {
                    // 代表执行失败
                    throw new UnknownException("图片上传失败");
                }
