...
import com.example.alcoholic.action.StatusAction;
import com.example.alcoholic.aop.Permissions;
import com.example.alcoholic.common.MyActivity;
import com.example.alcoholic.constant.JumpDataContants;
import com.example.alcoholic.other.GridSpaceDecoration;
import com.example.alcoholic.ui.acitivity.MainActivity;
import com.example.alcoholic.ui.adapter.ImageSelectAdapter;
import com.example.alcoholic.ui.dialog.AlbumDialog;
import com.example.alcoholic.widget.HintLayout;
...
public final class ImageSelectActivity extends MyActivity
    ...
    public static void start(BaseActivity activity, int maxSelect, OnPhotoSelectListener listener) {
        ...
        intent.putExtra(JumpDataContants.AMOUNT, maxSelect);
        activity.startActivityForResult(intent, (resultCode, data) -> {
            ...
            if (resultCode == RESULT_OK) {
                lubanStudio(activity,listener,data.getStringArrayListExtra(JumpDataContants.IMAGE));
            } else {
                listener.onCancel();
            }
        });
    ...
    protected void initData() {
        // 获取最大的选择数
        mMaxSelect = getInt(JumpDataContants.AMOUNT, mMaxSelect);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.fab_image_select_floating) {
            if (mSelectImage.isEmpty()) {
                ...
                // 完成选择
                setResult(RESULT_OK, new Intent().putStringArrayListExtra(JumpDataContants.IMAGE, mSelectImage));
                finish();
            }
        }
    }
     ...
     * 鲁班压缩一下
     */
    private static void lubanStudio(BaseActivity activity,OnPhotoSelectListener listener, List<String> data) {
        ...
        Flowable.just(data)
                ...
                .map(new Function<List<String>, List<File>>() {
                    @Override
                    public List<File> apply(List<String> list) throws Throwable {
